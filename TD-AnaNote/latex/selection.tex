...
We use $B_s\to D_s\pi\pi\pi$ data that passes all selection steps, including the trigger and stripping selection, offline cuts and physical background vetoes, as signal proxy. 
...
\begin{itemize} 
	...
	\item  the normalized difference between the transverse momentum
	...
	\item logarithm of the the smallest $X_{d}$ daughter impact-parameter $\chi^{2}$,  $X_{s}$ log(min($\chi^{2}_{IP}$))

	\item largest distance of closest approach of the $X_{d}$ daughters, max(DOCA)
