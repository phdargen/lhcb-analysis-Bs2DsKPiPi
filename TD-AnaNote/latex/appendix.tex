...
\input{tables/MassFit/yields_norm_run1.tex} 

\input{tables/MassFit/yields_norm_run2.tex}
\label{tab:massFitNormA}
\end{table}
\begin{table}[h]
\centering
\caption{Signal and background yields for the $B_s \to D_s K \pi \pi$ sample split by data-taking period.}
\input{tables/MassFit/yields_signal_run1.tex} 

\input{tables/MassFit/yields_signal_run2.tex}
...
\begin{figure}[h]
\centering
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_1.pdf}
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_2.pdf}
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_3.pdf}

\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_4.pdf}
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_5.pdf}
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_6.pdf}

\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_7.pdf}
\includegraphics[height=!,width=0.32\textwidth]{figs/Resolution/SignalData_18_bin_8.pdf}


\caption{Decay-time distribution for fake $B_s$ candidates from promptly produced $D_s$ candidates, combined with random prompt $K\pi\pi$ bachelor tracks, for bins in the per-event decay time error estimate. Data taken in 2018.}
\label{fig:}
\end{figure}
...
\centering
\small
\caption{Measured time resolution for prompt-$D_s$ data in bins of the per-event decay time error estimate. Data taken in 2018.}
\input{tables/Resolution/ResoTable_Data_18.txt}
\label{table:ResoParamsData_18}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[height=!,width=0.45\textwidth]{figs/Acceptance/timeAcc_comparison_by_year_adaptive_N4.pdf}
...
\begin{table}[h]
    \caption{Intermediate-state components considered for the $B_s \to D_s K \pi \pi$  LASSO model building procedure. The letters in square brackets refer to the relative orbital angular momentum of
the decay products. 
   If no angular momentum is specified, the lowest angular momentum state compatible with angular momentum conservation and, where appropriate, parity conservation, is used.}
 \label{tab:ampsDsKpipiLASSO}
\footnotesize
	\renewcommand{\arraystretch}{1.25}
    \begin{subtable}{.4\linewidth}
      \centering
%           \footnotesize
        \caption{\small Cascade decays}
        \begin{tabular}{ll}
        \hline
              $B_s \to D_s^- \, [K_{1}(1270)^{+}[S,D]\to \pi^+ \, K^{*}(892)^{0}]$ \\
              $B_s \to D_s^- \, [K_{1}(1270)^{+}\to \pi^+ \, K_0^{*}(1430)^{0}]$ \\
...
%
              ...
              $B_s \to D_s^- \, [K_{2}(1770)^{+}\to K^+ \, \rho(770)^{0}]$ \\
               $B_s \to D_s^- \, [K_{2}(1770)^{+}\to \pi^+ \, K_2^{*}(1430)^{0}]$ \\
              $B_s \to D_s^- \, [K_{2}(1770)^{+}\to K^+ \, f_2(1270)^0]$ \\
%
               $B_s \to D_s^- \, [K_{2}^*(1980)^{+}\to \pi^+ \, K^{*}(892)^{0}]$ \\
              $B_s \to D_s^- \, [K_{2}^*(1980)^{+}\to K^+ \, \rho(770)^{0}]$ \\
               $B_s \to D_s^- \, [K_{2}^*(1980)^{+}\to \pi^+ \, K_2^{*}(1430)^{0}]$ \\
              $B_s \to D_s^- \, [K_{2}^*(1980)^{+}\to K^+ \, f_2(1270)]$ \\
               $B_s \to K^+ [ D_{s1}(2536)^-\to  D_s^- \,  \, \rho(770)^{0}]$ \\
               $B_s \to K^+ [ D_{s1}(2536)^-\to  D_s^- \,  \, \sigma^0]$ \\
        \hline
	  \end{tabular}
    \end{subtable}%
    \hfill
    \begin{subtable}{.4\linewidth}
      \centering
%                 \footnotesize
        \caption{\small Quasi two-body decays}
        \begin{tabular}{ll}
        \hline
              $B_s \to \sigma^{0} (D_s^- K^+)_S$ \\
              $B_s \to \sigma^{0} (D_s^- K^+)_V$ \\
              $B_s \to \rho(770)^{0} (D_s^- K^+)_S$ \\
              $B_s[S,P,D] \to \rho(770)^{0} \, (D_s^- K^+)_V$\\
              $B_s \to  f_2(1270)^0 (D_s^- K^+)_S$ \\
%
%              
%              
...
%
              $B_s \to  K_0^{*}(1430)^{0} \, (D_s^- \pi^+)_S$ \\
%              $B_s \to  K^{*}(1430)^{0} \, (D_s^- \pi^+)_V$ \\
              $B_s \to  K_2^{*}(1430)^{0} \, (D_s^- \pi^+)_S$ \\
...
for 200 time-dependent amplitude fits with randomized starting parameters.
%Less than 10\% of the fits did not converge.
The majority of the fits (66\%) converge to the global minimum. 
Three well isolated local minima are detected 
at $\Delta(2NLL) = 14$ ($3.7\sigma$),
$\Delta(2NLL) = 23$ ($4.8\sigma$)
and $\Delta(2NLL) = 30$ ($5.5\sigma$)
to which 8\%, 4\% and 3\%
of the fits converge, respectively.
The fits at $\Delta(2NLL) = 1,2,4,8,15,20,26,27,34,36,39$ did not converge properly to one of the nearby local minima. 
The remaining fits (12\%) are not converged or have $\Delta(2NLL) > 60$ and are not show.
...
%\resizebox{\linewidth}{!}{
	...
	\input{tables/fullFit/signal/int_fraction_table.tex}
%}
...
%\resizebox{\linewidth}{!}{
	\renewcommand{\arraystretch}{1.5}
	\input{tables/fullFit/signal/int_fraction_bar_table.tex}
%}
