...
\input{tables/ProductionAsym/Table.tex}
...
Here, only the decay modes $\Bs\to ( D_s \to KK \pi)\kaon\pion\pion$ and $\Bs\to (  D_s \to \pi\pi\pi)\kaon\pion\pion$ are considered.
For $\Bs\to ( \Ds \to K \pi \pi)\kaon\pion\pion$,
the contributions of the two oppositely
charged kaons cancel each other and a detection asymmetry of zero with the same uncertainty as determined for $D_s \to KK \pi$ and $D_s \to \pi\pi\pi$ is assumed.
In contrast, only the decay mode $D_s \to K \pi\pi$ is a source of detection asymmetry for $\Bs \to \Ds \pi \pion\pion$
decays resulting in an average detection asymmetry of 
$A^{det}(\Km\pip) = -0.07 \pm 0.15 \%$ (Run-I) 
and $A^{det}(\Km\pip) = -0.08 \pm 0.21 \%$ (Run-II).
...
\hline
\end{tabular}
\caption{Summary of the $\Km\pip$ detection asymmetries for $\Bs\to \Ds K \pion\pion$ decays obtained from the fits to the Run-I and Run-II calibration samples.}
